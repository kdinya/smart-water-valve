...
A custom Home Assistant Lovelace card for a smart water shut-off valve, with animated flowing water, a leak-sensor list, battery + signal indicators, an animation kill-switch, and full UK/RU/EN localization.

Current version: **v5.0.0**. See [Changelog](#changelog) for what's new.
...
- Displays an **unlimited list of leak sensors** (each with an optional name), each shown as DRY/LEAK with an icon that changes color and pulses on leak. **A leak block is shown whenever its entity is set — the name is purely optional/cosmetic.** The visual editor currently offers up to 2 rows to keep the card layout tidy, but the config format itself has no limit.
- Shows a **battery level** and a **signal level** for the valve actuator side by side, if you provide the corresponding sensors.
- Lets you **disable all animations** except the crane-toggle motion itself — useful on low-power dashboards/kiosks or for people sensitive to motion.
- Lets you set a **fixed or minimum card height**, if you need it to line up with other cards on your dashboard.
...
- Optionally: a `sensor`/`binary_sensor` reporting the valve's real physical state, a battery `sensor`, a signal-level `sensor`, and any number of `binary_sensor` leak/moisture sensors (2 rows exposed in the visual editor for now).
...
7. Open the browser console (F12) and confirm you see a `WATER-VALVE-CARD` / `5.0.0` log line — this confirms the right version is active.
...
3. The card's built-in editor opens, letting you pick every entity and label from dropdowns — no YAML needed. Fields appear in this order: language, name, valve switch, valve state sensor, battery sensor, signal sensor, text/button overrides, toggle animation timing, animation on/off, card height/min-height, then a **Leak sensors** section below the form with a name+entity row per sensor and a "+ Add leak sensor" button. See the [Configuration reference](#configuration-reference) below for what each field does.
...
| `kran_signal_entity` | `string` (entity id) | ❌ | A `sensor` entity with the signal level/quality of the valve actuator. Accepts either a `0-100`-style or a dBm-style (~`-100..0`) reading and maps it onto a 4-bar icon; the raw value + its own `unit_of_measurement` is shown as text. | none (signal indicator hidden) |
| `leak_sensors` | `array` of `{ label, entity }` | ❌ | The list of leak/moisture sensor blocks. `entity` (a `binary_sensor`) is what makes a block appear at all; `label` is purely cosmetic and can be left empty. No hard limit in the config format — the visual editor currently offers up to 2 rows. | `[]` (no leak blocks shown) |
...
| `disable_animations` | `boolean` | ❌ | Turns off every animation on the card — leak glow/pulse, sensor shake, ripple, the flowing-water canvas — **except** the crane-toggle motion itself, which keeps working regardless. | `false` |
| `card_height` | `number` (px, 100–1000) | ❌ | Fixed current height for the card. Leave empty for the card's natural auto height. | none (auto) |
| `card_min_height` | `number` (px, 100–1000) | ❌ | Minimum height for the card — it can still grow taller (e.g. with more leak sensor rows) but won't shrink below this. Leave empty for no minimum. | none (auto) |

> A leak sensor block is controlled **only by its `entity` field**. If it's set, the block is shown — with a name if `label` is set, or without one if it's left empty. If it's not set, the block doesn't appear at all, regardless of the label.

> **Upgrading from v4.x:** the old `bathroom_label`/`bathroom_leak_entity`/`kitchen_label`/`kitchen_leak_entity` fields are automatically migrated into `leak_sensors` the first time the card loads with your existing config — nothing is lost. The old fields are dropped the next time you save from the visual editor.
...
kran_signal_entity: sensor.kran_signal
leak_sensors:
  - label: Bathroom
...
- **Hold (long-press, ~500ms)** the action button, any leak-sensor block, the battery indicator, or the signal indicator to open that entity's more-info dialog. A quick tap still just does its normal thing.
...
- If **any** configured leak sensor turns on, the whole card switches to an emergency visual state: red glowing/pulsing border, animated leak drops inside the pipes, and the shut-off button pulses to draw attention. If more than one sensor is triggered simultaneously, the status text reflects that explicitly.
- The **battery** and **signal** indicators sit side by side in the header; each is hidden individually if you don't configure its entity.
- With `disable_animations: true`, everything above stops moving — leak glow, sensor shake, ripple, the flowing-water canvas — **except** the crane-toggle motion, which always keeps working since it's the one animation that actually confirms the valve did something.
- The water/bubble canvas animation automatically pauses when the browser tab is hidden, the card scrolls out of view, or less than 30% of it is visible, and resumes once it's meaningfully on-screen again (unless animations are disabled entirely, in which case it stays on a single static frame).

## Changelog

### v5.0.0

- **Added:** a signal-level indicator (`kran_signal_entity`), shown next to the battery indicator in the header.
- **Added:** `disable_animations` — turns off every animation on the card (leak glow/pulse, sensor shake, ripple, the flowing-water canvas) except the crane-toggle motion, which always keeps working.
- **Added:** `card_height` / `card_min_height` to fix the card at, or floor it above, a given pixel height.
- **Changed:** leak sensors are now an unlimited `leak_sensors: [{ label, entity }, ...]` list instead of the fixed `bathroom_*`/`kitchen_*` pair. The visual editor exposes up to 2 rows for now (with a "+ Add leak sensor" button), but the config format itself has no cap.
- **Added:** automatic migration — existing `bathroom_label`/`bathroom_leak_entity`/`kitchen_label`/`kitchen_leak_entity` config is converted into `leak_sensors` the first time the card (or its editor) loads, so upgrading doesn't lose anyone's setup. The old fields are dropped the next time you save from the visual editor.
- **Changed:** the leak status line now reads a plain "leak on multiple sensors" message when more than one is triggered at once, instead of a message hard-coded to exactly two sensors.
...
| A leak block doesn't show even though I set the entity | Double-check the entity id is spelled correctly, and that it's a row in `leak_sensors` (or was migrated from the old `*_leak_entity` fields) — the block always shows once a valid entity is set, with or without a label. |
