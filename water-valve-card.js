// ═══════════════════════════════════════════════════════════════
//  Water Valve Card  v6.4 — труби з водою на всю ширину,
...
class WaterValveCard extends HTMLElement {
  ...
  setConfig(config) {
    if (!config.switch_entity) throw new Error('Потрібно вказати switch_entity');
    this._config = {
      switch_entity: config.switch_entity,
      valve_state_entity: config.valve_state_entity || null,
      kran_battery_entity: config.kran_battery_entity || 'sensor.kran_battery',
      name: config.name || 'Водяний кран',
      bathroom_leak_entity: config.bathroom_leak_entity || 'binary_sensor.datchik_ppotiechki_vanna_water_leak',
      kitchen_leak_entity: config.kitchen_leak_entity || 'binary_sensor.datchik_ppotiechki_kukhnia_water_leak',
  ...
  _isLeak(entityId) { return this._state(entityId) === 'on'; }

  _getValveState() {
    if (!this._hass) return { switchState: 'unknown', valveState: 'unknown' };
    ...
    const valveState = vs?.state || (switchState === 'on' ? 'open' : switchState === 'off' ? 'closed' : 'unknown');
    return { switchState, valveState };
  }
  ...
  _toggle() {
    ...
    const { switchState } = this._getValveState();
    this._isToggling = true;
    this._targetState = switchState === 'on' ? 'closed' : 'open';
    const service = switchState === 'on' ? 'turn_off' : 'turn_on';
  ...
  _render() {
    ...
    const batteryState = this._state(cfg.kran_battery_entity);
    ...
    this._updateSensor(r, 'bath', cfg.bathroom_label, d.bathLeak, cfg.text_dry, cfg.text_leak);
    this._updateSensor(r, 'kitchen', cfg.kitchen_label, d.kitLeak, cfg.text_dry, cfg.text_leak);
  ...
  _updateSensor(root, id, label, isLeak, textDry, textLeak) {
    const el = root.getElementById(`sensor-${id}`);
    const icon = root.getElementById(`icon-${id}`);
    el.classList.toggle('leak', isLeak);
    root.getElementById(`label-${id}`).textContent = label.toUpperCase();
...
window.customCards.push({
  ...
  description: 'Кран з реалістичною водою на всю ширину, центральний корпус фіксований.',
  preview: true,
});