// ═══════════════════════════════════════════════════════════════
//  Water Valve Card  v3.2.0 — труби з водою на всю ширину,
...
class WaterValveCardEditor extends HTMLElement {
  ...
  _fire() {
    const cfg = { ...this._config };
    for (const k of ['switch_entity','valve_state_entity','kran_battery_entity','bathroom_leak_entity','kitchen_leak_entity','bathroom_label','kitchen_label']) {
      if (!cfg[k]) delete cfg[k];
    }
    if (!cfg.language) cfg.language = 'en';
    if (!cfg.name) {
      const pack = (window.customElements.get('water-valve-card') && WaterValveCard.I18N[cfg.language]) || null;
      cfg.name = pack ? pack.default_name : 'Water valve';
    }
  ...
  _row(label, node) {
    ...
    lab.style.cssText = 'font-weight:500;margin-bottom:6px;';
  ...
  _render() {
    ...
    root.style.cssText = 'padding:4px 0;';

    // Language first
    const lang = document.createElement('ha-selector');
    // fallback select if ha-selector unavailable
    const select = document.createElement('select');
    select.style.cssText = 'width:100%;padding:10px;border-radius:8px;font:inherit;';
    for (const [code, title] of [['uk','Українська (uk)'],['ru','Русский (ru)'],['en','English (en)']]) {
      const o = document.createElement('option');
      o.value = code; o.textContent = title;
      if ((this._config.language || 'en') === code) o.selected = true;
    ...
    root.appendChild(this._row('Language / Мова / Язык', select));

    root.appendChild(this._row('Card name / Назва', this._text('name', 'Name')));
    root.appendChild(this._row('Valve entity (required)', this._picker('switch_entity', ['switch', 'valve'])));
    root.appendChild(this._row('Valve state sensor (optional)', this._picker('valve_state_entity', ['sensor', 'binary_sensor'])));

    root.appendChild(this._row('Leak sensor 1 entity (optional)', this._picker('bathroom_leak_entity', ['binary_sensor'])));
    root.appendChild(this._row('Leak sensor 1 name (optional — empty = hidden)', this._text('bathroom_label', 'e.g. Bathroom')));
    root.appendChild(this._row('Leak sensor 2 entity (optional)', this._picker('kitchen_leak_entity', ['binary_sensor'])));
    root.appendChild(this._row('Leak sensor 2 name (optional — empty = hidden)', this._text('kitchen_label', 'e.g. Kitchen')));

    root.appendChild(this._row('Battery sensor (optional)', this._picker('kran_battery_entity', ['sensor'])));

    const dur = document.createElement('ha-textfield');
    dur.style.width = '100%';
    dur.label = 'Animation duration (ms)';
    dur.type = 'number';
    dur.value = this._config.toggle_lock_ms || 8000;
    dur.addEventListener('change', (ev) => {
      this._config = { ...this._config, toggle_lock_ms: parseInt(ev.target.value, 10) || 8000 };
      this._fire();
    });
    root.appendChild(this._row('Valve animation time (ms)', dur));

    const autoWrap = document.createElement('label');
    autoWrap.style.cssText = 'display:flex;align-items:center;gap:8px;margin-bottom:12px;';
    const auto = document.createElement('input');
    auto.type = 'checkbox';
    auto.checked = !!this._config.auto_toggle_duration;
    auto.addEventListener('change', (ev) => {
      this._config = { ...this._config, auto_toggle_duration: ev.target.checked };
      this._fire();
    });
    autoWrap.appendChild(auto);
    autoWrap.appendChild(document.createTextNode('Auto-measure animation time (learns on first toggles)'));
    root.appendChild(autoWrap);

    const hint = document.createElement('div');
    hint.style.cssText = 'opacity:0.65;font-size:12px;line-height:1.4;';
    hint.innerHTML = 'Leak blocks show only when <b>entity + name</b> are both set.<br>0 / 1 / 2 sensors supported.';
...
window.customCards.push({
  ...
  description: 'Water valve card with language, optional leak sensors, configurable animation.',
  preview: true,
});