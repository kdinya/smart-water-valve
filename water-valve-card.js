// ═══════════════════════════════════════════════════════════════
//  Water Valve Card  v3.2.1 — труби з водою на всю ширину,
...
class WaterValveCard extends HTMLElement {
  ...
  static async getConfigElement() {
    // Ensure editor is defined before HA mounts it
    if (!customElements.get("water-valve-card-editor")) {
      defineWaterValveCardEditor();
    }
    await customElements.whenDefined("water-valve-card-editor");
...
function defineWaterValveCardEditor() {
  if (customElements.get("water-valve-card-editor")) return;
  ...
  class WaterValveCardEditor extends HTMLElement {
    ...
    _schema() {
      return [
        ...
        },
        {
          ...
          selector: { number: { min: 500, max: 120000, mode: "box", unit_of_measurement: "ms" } },
        },
        {
    ...
    _build() {
      ...
      this._form.addEventListener("value-changed", (ev) => {
        ...
        if (cfg.toggle_lock_ms === undefined || cfg.toggle_lock_ms === null || cfg.toggle_lock_ms === "") {
          cfg.toggle_lock_ms = 8000;
        }
        this._config = cfg;
        this.dispatchEvent(
    ...
    _updateForm() {
      ...
      this._form.data = {
        ...
        switch_entity: this._config.switch_entity || "",
        valve_state_entity: this._config.valve_state_entity || "",
        bathroom_leak_entity: this._config.bathroom_leak_entity || "",
        bathroom_label: this._config.bathroom_label || "",
        kitchen_leak_entity: this._config.kitchen_leak_entity || "",
        kitchen_label: this._config.kitchen_label || "",
        kran_battery_entity: this._config.kran_battery_entity || "",
        toggle_lock_ms: this._config.toggle_lock_ms ?? 8000,
        auto_toggle_duration: !!this._config.auto_toggle_duration,
      };
    }
  }
...
// Define editor first, then card
defineWaterValveCardEditor();

if (!customElements.get("water-valve-card")) {
  customElements.define("water-valve-card", WaterValveCard);
} else {
  // Hot reload / HACS update path
  try {
    customElements.define("water-valve-card", WaterValveCard);
  } catch (e) {
...
if (!window.customCards.find((c) => c.type === "water-valve-card")) {
  window.customCards.push({
    type: "water-valve-card",
    name: "Water Valve Card",
    description: "Water valve with language, optional leak sensors, animation timing.",
    preview: true,
...
console.info(
  "%c WATER-VALVE-CARD %c 3.2.1 ",
  "color:#fff;background:#0ea5e9;font-weight:700",
  "color:#0ea5e9;background:#1f2937;font-weight:700"
);
