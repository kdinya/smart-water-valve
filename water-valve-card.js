// ═══════════════════════════════════════════════════════════════
//  Water Valve Card  v3.1.0 — труби з водою на всю ширину,
...
class WaterValveCardEditor extends HTMLElement {
  ...
  _fire() {
    const cfg = { ...this._config };
    for (const k of ['switch_entity','valve_state_entity','kran_battery_entity','bathroom_leak_entity','kitchen_leak_entity']) {
      if (!cfg[k]) delete cfg[k];
    }
    if (!cfg.name) cfg.name = 'Водяний кран';
  ...
  _render() {
    ...
    name.label = 'Назва';
    name.value = this._config.name || 'Водяний кран';
    ...
    root.appendChild(this._row('Назва картки', name));

    root.appendChild(this._row('Кран (обовʼязково)', this._picker('switch_entity', ['switch', 'valve'])));
    root.appendChild(this._row('Сенсор стану (опційно)', this._picker('valve_state_entity', ['sensor', 'binary_sensor'])));
    root.appendChild(this._row('Датчик протічки 1 (опційно)', this._picker('bathroom_leak_entity', ['binary_sensor'])));
    root.appendChild(this._row('Датчик протічки 2 (опційно)', this._picker('kitchen_leak_entity', ['binary_sensor'])));
    root.appendChild(this._row('Батарея (опційно)', this._picker('kran_battery_entity', ['sensor'])));

    const hint = document.createElement('div');
    hint.style.cssText = 'opacity:0.65;font-size:12px;';
    hint.textContent = 'Без датчиків — блоки сховані. Один — один блок. Два — два блоки.';
...
window.customCards.push({
  ...
  description: 'Форма вибору крана і опційних датчиків протічки.',
  preview: true,
});