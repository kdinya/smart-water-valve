// ═══════════════════════════════════════════════════════════════
//  Water Valve Card  v3.4.0 — труби з водою на всю ширину,
...
class WaterValveCard extends HTMLElement {
  ...
  static getStubConfig() {
    // Intentionally NO entities — user picks them in the visual editor
    return {
      language: "uk",
...
function defineWaterValveCardEditor() {
  ...
  class WaterValveCardEditor extends HTMLElement {
    ...
    setConfig(config) {
      this._config = this._clean({ language: "uk", name: "Smart Water Valve", ...(config || {}) });
      this._render();
    }
    ...
    _schema() {
      return [
        ...
        },
        {
          ...
          label: "Leak sensor 1 (optional)",
          selector: { entity: { domain: "binary_sensor" } },
        },
        {
          name: "bathroom_label",
          label: "Leak sensor 1 name (required to show)",
          selector: { text: {} },
        },
        {
          name: "kitchen_leak_entity",
          label: "Leak sensor 2 (optional)",
          selector: { entity: { domain: "binary_sensor" } },
        },
        {
          name: "kitchen_label",
          label: "Leak sensor 2 name (required to show)",
          selector: { text: {} },
        },
        {
    ...
    _render() {
      if (!this.isConnected && !this._form) {
        // still build when HA sets config before connect
      }
      if (!this._form) {
        this.innerHTML = "";
      ...
      // Only show keys we manage — never prefill entities
...
// Always re-register picker metadata (helps after HACS update)
...
window.customCards.push({
  ...
  description: "Valve card — pick your entities in the visual editor (no defaults).",
  preview: true,
...
try {
  ...
  console.warn(
    "water-valve-card already defined — hard refresh the browser (Ctrl+F5) to load v3.4.0",
    err
  );
}

console.info(
  "%c WATER-VALVE-CARD %c v3.4.0 ",
  "color:#fff;background:#0284c7;font-weight:700;padding:2px 6px;",
  "color:#0284c7;background:#0f172a;font-weight:700;padding:2px 6px;"
);
